...
const CommitmentForm: React.FC<CommitmentFormProps> = ({ 
  ...
  const availablePIs = useMemo(() => {
    ...
    return pis.filter(pi => {
      ...
      return piCredits.some(c => getNCBalance(c) > 0.01);
    });
  ...
  const availableNDs = useMemo(() => {
    ...
    return nds.filter(nd => {
      ...
      return ndCredits.some(c => getNCBalance(c) > 0.01);
    });
  }, [credits, formData.ug, formData.pi, getNCBalance]);

  // NCs Disponíveis ordenadas por antiguidade (FIFO) com saldos dinâmicos
  const availableNCs = useMemo(() => {
    ...
    return credits
      ...
      .map(c => ({
        ...c,
        realBalance: getNCBalance(c)
      }))
      .filter(c => c.realBalance > 0.01)
      .sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
  }, [credits, formData.ug, formData.pi, formData.nd, getNCBalance]);
  ...
  const handleManualAllocChange = (ncId: string, val: number) => {
    ...
    if (!nc) return;
    const safeVal = Math.min(nc.realBalance, Math.max(0, val));
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             ...
             <div className="space-y-2">
                ...
                >
                  <option value="">{availablePIs.length === 0 ? (!formData.ug ? 'Aguardando UG...' : 'Sem PI com saldo') : 'Selecione o PI'}</option>
             ...
             <div className="space-y-2">
                ...
                >
                  <option value="">{availableNDs.length === 0 ? (!formData.pi ? 'Aguardando PI...' : 'Sem ND com saldo') : 'Selecione a ND'}</option>
             ...
             <div className="bg-slate-50 rounded-[2.5rem] border border-slate-200 p-8 flex flex-col min-h-[500px] shadow-inner">
                <div className="flex items-center justify-between mb-8 border-b border-slate-200 pb-4">
                   <h3 className="text-[11px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-3">
                      ...
                      Composição do Saldo (Cálculo Real FIFO)
                   </h3>
                   <span className="text-[10px] font-black text-slate-400 uppercase bg-white px-3 py-1 rounded-full border border-slate-100">{availableNCs.length} NCs</span>
                </div>
                   ...
                   {availableNCs.length > 0 ? availableNCs.map((nc, idx) => (
                     <div key={nc.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between group hover:border-emerald-500 transition-all">
                        <div className="flex-1">
                           ...
                           <p className="text-[8px] font-bold text-slate-400 mt-2 uppercase">Disponível: {formatCurrency(nc.realBalance)}</p>
                     ...
                     <div className="h-full flex flex-col items-center justify-center opacity-20 text-center px-12">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] leading-relaxed">
                          Aguardando seleção de UG/PI/ND para exibir Notas de Crédito com saldo disponível real
                        </p>
                     </div>
                   )}
                </div>
