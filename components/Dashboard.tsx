...
const Dashboard: React.FC<DashboardProps> = ({ credits, commitments, refunds, cancellations, filters, setFilters }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    // Identificar Alertas Críticos - AGORA EXCLUI SALDO ZERO
    ...
    return {
      ...
      executionPercentage,
      barChartData,
      criticalAlerts: criticalAlerts.slice(0, 10) // Aumentado limite de visualização
    };
  ...
  const getIndividualNCBalance = (credit: Credit) => {
    ...
    return credit.valueReceived - totalSpent - refunded + cancelled;
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-black">
      ...
      {/* Modal de Detalhes da NC (Reutilizado da CreditList para consistência) */}
