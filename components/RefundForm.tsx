...
const RefundForm: React.FC<RefundFormProps> = ({ credits, commitments, refunds, cancellations, onSave, onCancel }) => {
  ...
  // NCs filtradas por dotação E COM SALDO DISPONÍVEL
  const filteredCredits = useMemo(() => {
    ...
    return credits
      ...
      .filter(c => c.available > 0.01); // FILTRO SOLICITADO: Apenas NCs com saldo
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              ...
              >
                <option value="">{!formData.ug ? 'Aguardando UG...' : 'Selecione o PI...'}</option>
            ...
            >
              ...
              {filteredCredits.map(c => (
                <option key={c.id} value={c.id}>
                  {c.nc} - SALDO: R$ {c.available.toLocaleString('pt-BR')}
                </option>
              ))}
            </select>
          </div>
